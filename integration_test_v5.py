"""V5.0 integration smoke test: full AI-vs-AI game using real GameRoom + ai_player.

Validates the new V5 phases: MARKET, LOCKDOWN_PLACE.
"""
import sys
import traceback
import ai_player
...
class AIDriver:
    ...
    def step(self):
        ...
        if not ok:
            print(f'[ERR] phase {r.phase} action {action} failed: {err}')
            return False
        return True


def run_one_game(seed=None):
    import random
    if seed is not None:
        random.seed(seed)
    r = GameRoom('TEST')
    ...
    while step_count < max_steps:
        if r.phase == 'GAME_OVER':
            break
        if r.phase == 'COLLISION_FLIP' and r.col_state and r.col_state['done']:
            r._finish_collision()
            break
        ok = driver.step()
        if not ok:
